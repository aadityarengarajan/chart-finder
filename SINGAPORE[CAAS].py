def getCharts(icao):
    ...
    url = 'https://aim-sg.caas.gov.sg/aip/2021-01-14/final/2021-01-14/html/index-en-GB.html'
    headers = {'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36'}
    r = requests.get(url,verify=False,headers=headers)
    lst = BeautifulSoup(r.content,'html.parser')
    lnk = lst.find('frame',{'name':'eAISNavigationBase'})['src']
    lnk = 'https://aim-sg.caas.gov.sg/aip/2021-01-14/final/2021-01-14/html/'+lnk
    ...
    for i in possibilities:
        if (icao in i) and ('.html' in i):
            return (('https://aim-sg.caas.gov.sg/aip/2021-01-14/final/' + str(i).split('?')[0].replace('eAIP/../','').replace('../','')))
    return 'Not Found'
